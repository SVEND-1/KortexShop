...
async function handleRegistration(e) {
    try {
        ...
        if (response.ok) {
            ...
            // Сохраняем данные в sessionStorage только для текущей сессии
...
async function handleCodeConfirmation() {
    try {
        ...
        // Форматируем код в формат, который ожидает сервер (6 цифр без дефиса)
        ...
        const operationId = type === 'registration' ? data.registrationId : data.resetId;

        console.log(`Подтверждение кода для ${type}:`, { operationId, code: formattedCode });

        const response = await fetch(`${endpoint}?${type === 'registration' ? 'registrationId' : 'resetId'}=${operationId}&code=${formattedCode}`, {
            method: 'POST'
        });
        ...
        if (response.ok) {
            ...
            if (type === 'registration') {
                // Регистрация завершена - сервер автоматически выполняет логин
                sessionStorage.removeItem('pendingRegistration');

                // Проверяем, есть ли redirectUrl в ответе
                if (result.redirectUrl) {
                    window.location.href = result.redirectUrl; // Главная страница
                } else {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (result.redirectUrl) {
                ...
                window.location.href = '/login'; // На страницу входа
            }
        } else {
...
async function handleResendCode() {
    try {
        ...
        let endpoint, storageKey, idParam;
        ...
        const response = await fetch(`${endpoint}?${idParam}=${operationId}`, {
            method: 'POST'
        });
